...
public class RegularExpressionExam {
	...
	public static void main(String[] args) {
		 ...
		 * 예를 들면 html에서 전화번호나 이메일 주소만 추출하거나 입력한 비밀번호가 숫자와 영문자의 조합으로 되어있는지 등을
		 * 쉽게 간단하게 확인할 수 있다.
		 * 
		 *  문자열 배열에 담긴 문자열 등 중에서 지정한 정규식과 일치하는 문자열을 찾아서 출력
		 *	Pattern은 정규식을 정의하고 Matcher는 비교하는 역할을 한다.
		 *	1. 정규식을 매개변수로 Pattern 클래스의 static 메서드 인 pattern.compile() 호출하여 Pattern 객체를 얻는다.
		 *	2. 정규식으로 비교할 대상을 매개변수 Pattern 클래스의 Matcher를 호출하여 객체를 얻는다.
		 *	3. Matcher 객체에 matches() 호출하여 정규식에 부합하는지 확인한다.
		 */

		String[] data = {"bat", "body", "bonus", "cA", "ca", "co", "c.","c0", "car","combat","count"
			,"date","dict"};
		 
//		Pattern p = Pattern.compile("c[a-z]*"); //c로 시작하는 소문자 영단어
//		Pattern p = Pattern.compile("c[a-z]"); //c로 시작하는 두 자리 소문자 영단어
//		Pattern p = Pattern.compile("c[a-zA-Z]"); //c로 시작하는 두 자리 대소문자 영단어
//		Pattern p = Pattern.compile("c[a-zA-Z0-9]"); //c로 시작하는 두 자리 대소문자,숫자까지 영단어
//		Pattern p = Pattern.compile(".*"); //모든 문자열
//		Pattern p = Pattern.compile("c."); //c로 시작하는 두 자리 문자열
//		Pattern p = Pattern.compile("c.*"); //c로 시작하는 전체 문자열
//		Pattern p = Pattern.compile("c\\."); //c로 시작하고 문자 .(\\는 기능역할을 하는게 아닌 문자열 역할을 하게 만듬)
